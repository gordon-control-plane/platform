...
async def test_concurrent_workflow_creation(client):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as async_client:
        ...
        for i in range(50):
            ...
            tasks.append(async_client.post("/api/workflows", json=payload, headers=headers))
...
def test_chaos_missing_headers(client):
    ...
    """
    # Missing CSRF
    response = client.post("/api/workflows", json={"name": "test", "args": {}}, headers={"Tailscale-User-Login": "user@example.com"})
    assert response.status_code == 403

    # Missing Auth
    response = client.post("/api/workflows", json={"name": "test", "args": {}}, headers={"X-Requested-With": "XMLHttpRequest"})
...
def test_security_admin_endpoint_no_bypass(client):
    ...
    assert response.status_code in [403, 500] # 500 if allowlist is missing, 403 if it exists but user not in it
